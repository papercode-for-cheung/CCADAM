...
def load_DDIs():
    ...
    while line:
        lineArr = line.strip().split()
        lineArr.append(1)
        DDIs.append(list(map(lambda x: int(float(x)), lineArr)))
        DDIs.append(neg())
        line = fileIn.readline()
    return DDIs
...
def write_to_txt():
    ...
    for i, (trian_id, test_id) in tqdm(enumerate(kf.split(DDs))):
        ...
        for ids in trian_id:
            ...
            i_str = str(i+1)
            flag += 1
            filename = './txt0/Durg/train'+i_str+'.txt'
        ...
        for ids in test_id:
            DD = DDs[ids]
            i_str = str(i + 1)
            filename = './txt0/Durg/test' + i_str + '.txt'
